...
async function handleCalculate() {
  ...
  try {
    const { data } = await axios.post('/calculate', { firstNumber, secondNumber, operator }, { baseURL: import.meta.env.VITE_API_BASE });
...
<template>
  <div class="w-100 max-w-lg bg-white mx-auto min-h-screen max-h-screen pb-3 overflow-y-auto">
    <h1 class="h-10 flex justify-center items-center border-b border-b-gray-300 py-0.5 text-sm font-semibold">
      <span class="text-lg">Group 4 CalcApp </span>
      <span v-if="calcState === 'error'" class="uppercase text-red-500 font-bold"> - An error occured</span>
    </h1>
    <div class="h-24 bg-gray-100 text-3xl font-light flex justify-end items-end text-right p-1 overflow-y-auto">
      {{ result }}
    </div>
    ...
    <button
      ...
      :disabled="calcState === 'loading'"
      class="w-full max-w-xs mx-auto mb-3 flex justify-center items-center py-1 bg-red-600 text-white font-semibold text-lg disabled:bg-blue-300"
    >
      {{ calcState === 'loading' ? 'Calculating' : 'Calculate' }}
    </button>

    <h1 class="text-center border-b border-b-gray-300 py-0.5 text-sm font-semibold">History</h1>
    <div class="p-4 flex flex-col-reverse gap-5 overflow-y-auto">
      <div v-for="operation in history" class="flex flex-col">
        <div class="flex border border-gray-300">
          <div class="p-2 bg-gray-300 flex items-center">{{ operation.firstNumber }} {{ operation.operator }} {{ operation.secondNumber }} =</div>
